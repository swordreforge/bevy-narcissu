    ...
    pub fn get_or_insert_taffy_viewport_node(&mut self, ui_root_entity: Entity) -> taffy::NodeId {
        *self
            ...
            .or_insert_with(|| {
                ...
                // NOTE(rustc release-optimization workaround): `Style { ..default() }` struct-update
                // syntax is miscompiled in release mode: defaulted fields such as `min_size.height`
                // end up with a corrupt CompactLength tag (8 instead of AUTO=3), which then trips
                // taffy's `unreachable!` in `Dimension::maybe_resolve` during grid layout.
                // Build the default style first, then mutate the fields explicitly.
                let mut viewport_style = taffy::style::Style::default();
                viewport_style.display = taffy::style::Display::Grid;
                // Note: Taffy percentages are floats ranging from 0.0 to 1.0.
                // So this is setting width:100% and height:100%
                viewport_style.size = taffy::geometry::Size {
                    width: taffy::style_helpers::percent(1.0_f32),
                    height: taffy::style_helpers::percent(1.0_f32),
                };
                viewport_style.align_items = Some(taffy::style::AlignItems::Start);
                viewport_style.justify_items = Some(taffy::style::JustifyItems::Start);
